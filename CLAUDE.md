...
- **The unit-sanity check is deterministic code, not model judgment** (PRD §9.4, the hero feature): a count within 5% of the sum of per-session counts, with a distinct roster present, raises a `unit_suspicion`. In demo data it must flag "432 students served" and propose 61 unique students.
